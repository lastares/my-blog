...
function getTuzki(obj){
    ...
    if(tuzkiNumber){
        ...
        for (var i = 1; i < 41; i++) {
            ...
            str+='<img src="https://'+window.location.host+'/home/emote/tuzki/'+i+'.gif" title="'+alt[i-1]+'" alt="宋耀锋博客">';
        };
        tuzkiObj.html(str);
        tuzkiNumber=0;
    }else{
...
function reply(obj){
    ...
    var str='<div id="cancelComment" class="b-box-textarea"><div class="b-box-content" contenteditable="true" onfocus="delete_hint(this)">请先登陆后回复评论</div><ul class="b-emote-submit"><li class="b-emote">&nbsp;<i class="el el-reddit" onclick="getTuzki(this)"></i>&nbsp;<input style="height:30px;margin-left:-4px;" class="form-control b-email" type="text" name="email" placeholder="接收回复的email地址" value="'+userEmail+'"><div class="b-tuzki"></div></li><li class="b-submit-button"><input type="button" value="评 论" aid="'+aid+'" pid="'+pid+'" username="'+username+'" onclick="comment(this)"></li><li class="b-clear-float"></li></ul></div>';
    var parentObj=$(obj).parents('.comment-info').eq(0).append(str);
    // var cancel = '<button id="cancelBtn" onclick="killSelf();" style="background:#da1a8d;color:#f3efef;">取消</button>';
    // $(obj).append(cancel);
}
